...
function Teams() {
  ...
  const fetchTeams = async () => {
    try {
      const response = await axios.get("http://localhost:8011/api/teams");
  ...
  const openModal = (member = null) => {
    if (member) {
      ...
      setIsEditMode(false);
      setModalData({ name: "", email: "", password: "", role: "staff", status: "active", selectedSidebarLinks: [] });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        const response = await axios.put(`http://localhost:8011/api/teams/update/${modalData.id}`, payload);
        setTeams(teams.map((team) => (team._id === modalData.id ? response.data.member : team)));
      } else {
        if (!modalData.password) {
          alert("Password is required for new team members");
          return;
        }
        const response = await axios.post("http://localhost:8011/api/teams/create", payload);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this team member?")) {
      try {
        await axios.delete(`http://localhost:8011/api/teams/delete/${id}`);
  ...
  const handleStatusToggle = async (id, currentStatus) => {
    ...
    try {
      const response = await axios.put(`http://localhost:8011/api/teams/update/${id}`, { status: newStatus });
      setTeams(teams.map((team) => (team._id === id ? response.data.member : team)));
    } catch (error) {
      alert(error.response?.data?.error || "An error occurred");
    }
  };
        ...
        <tbody>
          {filteredTeams.map((team, index) => (
            <tr key={team._id} style={tbodyRowStyle}>
              <td style={tdStyle}>{String((currentPage - 1) * itemsPerPage + index + 1).padStart(2, "0")}</td>
              <td style={tdStyle}>
                <div
                ...
                >
                  <img
                    src={team.image ? team.image : `https://ui-avatars.com/api/?name=${encodeURIComponent(team.name)}&background=random&color=fff`}
                    alt="avatar"
                    style={avatarStyle}
                  />
                  <input
              ...
              <td style={tdStyle}>
                <span style={statusBadge}>{team.role.charAt(0).toUpperCase() + team.role.slice(1)}</span>
              </td>
              <td style={tdStyle}>
                <button
                  style={{
                    ...statusBadge,
                    backgroundColor: team.status === "active" ? "#ecfdf5" : "#fee2e2",
              ...
              <td style={{ ...tdStyle, textAlign: "center" }}>
                <button style={editBtn} onClick={() => openModal(team)} title="Edit">
                  <FaEdit />
                </button>
                <button style={deleteBtn} onClick={() => handleDelete(team._id)} title="Delete">
                  <FaTrash />
                </button>
              </td>
            </tr>
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: "flex", gap: "20px" }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={formGroupStyle}>
                    <label style={labelStyle}>Password {isEditMode && "(Optional)"}</label>
                    <input
                      type="password"
                    ...
                    <div style={{ maxHeight: "100px", overflowY: "auto" }}>
                      {sidebarLinks.map((link) => (
                        <div key={link.label} style={{ marginBottom: "8px" }}>
                          <input
                            type="checkbox"
                            checked={modalData.selectedSidebarLinks.includes(link.label)}
                            onChange={(e) => handleSidebarLinkChange(e, link.label)}
                          />
