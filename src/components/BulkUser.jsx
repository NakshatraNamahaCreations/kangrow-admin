...
const handleConfirmUpload = async () => {
  ...
  try {
    ...
    if (response.data.failedCount > 0) {
      setErrorMessage(
        `Some users failed. Count: ${response.data.failedCount}`
      );
      setShowErrorModal(true);
    } else {
      navigate("/user-data");
...
//     if (response.data.failedCount > 0) {
//   const failedDetails = response.data.failedUsers
//     .map(
//       (u, i) =>
//         `${i + 1}. ${u.Name || u.name} (${u.Phone || u.phoneNumber}) → ${u.reason}`
//     )
//     .join("\n");

//   setErrorMessage(
//     `❌ ${response.data.failedCount} user(s) could not be uploaded:\n\n${failedDetails}`
//   );
//   setShowErrorModal(true);
// } else {
//   navigate("/user-data");
